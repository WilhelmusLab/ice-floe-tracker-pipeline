...
@app.command()
def main(input_path: pathlib.Path, output_path: pathlib.Path, mask_value: int = 0):
    with rasterio.open(input_path) as in_dataset:
        ...
        hsv_tuples = {
            i: (i*1.0/max_value, 1, 1) for i in all_values
        }
        rgba_values = {
        ...
        new_profile.update(nodata=0, dtype="uint8", count=4, colorinterp=[ColorInterp.red, ColorInterp.green, ColorInterp.blue, ColorInterp.alpha] )
